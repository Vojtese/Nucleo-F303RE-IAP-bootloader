...
void xmodem_receive(void)
{
  ...
  while (X_OK == status)
  {
    ...
    uart_status comm_status = HAL_UART_Receive(&huart2, &header, 1u, UART_WAIT);


    /* Spam the host (until we receive something) with ACSII "C", to notify it, we want to use CRC-16. */
    if ((UART_OK != comm_status) && (false == x_first_packet_received))
    {
      (void)HAL_UART_Transmit(&huart2, "C", 1u, UART_WAIT);
    }
...
static xmodem_status xmodem_handle_packet(uint8_t header)
{
  ...
  comm_status |= HAL_UART_Receive(&huart2, &received_packet_number[0u], X_PACKET_NUMBER_SIZE,UART_WAIT);
  comm_status |= HAL_UART_Receive(&huart2, &received_packet_data[0u], size, UART_WAIT);
  comm_status |= HAL_UART_Receive(&huart2, &received_packet_crc[0u], X_PACKET_CRC_SIZE, UART_WAIT);
...
static xmodem_status xmodem_error_handler(uint8_t *error_number, uint8_t max_error_number)
{
  ...
  {
    /* Graceful abort. */
    (void)HAL_UART_Transmit(&huart2, X_CAN, 1u, UART_WAIT);
    (void)HAL_UART_Transmit(&huart2, X_CAN, 1u, UART_WAIT);
    status = X_ERROR;
  }
  ...
  {
    (void)HAL_UART_Transmit(&huart2, X_NAK, 1u, UART_WAIT);
    status = X_OK;
  }
  return status;
}
uart_status uart_receive(uint8_t *data, uint16_t length)
{
  ...
  if (HAL_OK == HAL_UART_Receive(&huart2, data, length, UART_WAIT))
  {
    status = UART_OK;
  }
...
uart_status uart_transmit_ch(uint8_t data)
{
  ...
  if (HAL_OK == HAL_UART_Transmit(&huart2, &data, 1u, UART_WAIT))
  {
    status = UART_OK;
  }
  return status;
...
uart_status uart_transmit_str(uint8_t *data)
{
  ...
  if (HAL_OK == HAL_UART_Transmit(&huart2, data, length, UART_WAIT))
  {
    status = UART_OK;
  }
